...
![TreeSet](../PNGFigures/TreeSet_UML.png)

![TreeMap](../PNGFigures/TreeMap_UML.png)
...
前文说到当查找树的结构发生改变时，红黑树的约束条件可能被破坏，需要通过调整使得查找树重新满足红黑树的约束条件。调整可以分为两类：一类是**颜色调整**，即改变某个节点的颜色；另一类是**结构调整**，即改变检索树的结构关系。结构调整过程包含两个基本操作：**左旋（Rotate Left），右旋（RotateRight）**。
